...
class MemoryItem:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "item_digest", canonical_digest({"memory_id": self.memory_id, "content": self.content, "tags": self.tags, "embedding": self.embedding, "metadata": self.metadata}))
...
class WorkingMemory:
    ...
    def remember(self, item: MemoryItem) -> tuple[MemoryItem, ...]:
        ...
        with self._lock:
            retained = tuple(row for row in self._items if row.memory_id != item.memory_id)
...
class EpisodicMemoryStore:
    ...
    def put(self, item: MemoryItem) -> MemoryItem:
        ...
        with self._lock:
            previous = self._items.get(item.memory_id)
    ...
    def search(self, query: str, *, limit: int = 10) -> tuple[MemoryItem, ...]:
        ...
        with self._lock:
            matches = tuple(
                ...
                if needle in item.content.casefold() or any(needle in tag.casefold() for tag in item.tags)
            )
    ...
    def search(self, query: tuple[float, ...], *, limit: int = 10) -> tuple[tuple[MemoryItem, float], ...]:
        ...
        with self._lock:
            ...
            for item in self._items.values():
                ...
                item_norm = math.sqrt(sum(float(value) ** 2 for value in item.embedding))
