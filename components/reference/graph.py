...
class GraphSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.next_nodes) is not tuple or any(
            ...
            raise ValueError("graph snapshot next_nodes must contain non-empty strings")
...
class SQLiteGraphCheckpointer(GraphCheckpointerPort):
    """Crash-durable graph snapshots keyed by thread identity."""
    ...
    def save(self, snapshot: GraphSnapshot) -> None:
        ...
        with self._lock:
            ...
            try:
                self._connection.execute(
                    "REPLACE INTO graph_snapshots (thread_id, checkpoint_id, snapshot_json) VALUES (?, ?, ?)",
                    (snapshot.thread_id, snapshot.checkpoint_id, canonical_text(document)),
                )
    ...
    def load(self, thread_id: str) -> GraphSnapshot | None:
        ...
        with self._lock:
            row = self._connection.execute(
                "SELECT checkpoint_id, snapshot_json FROM graph_snapshots WHERE thread_id = ?",
                (thread_id,),
            ).fetchone()
        if row is None:
            return None
        document = strict_json_loads(row[1])
        ...
        if not isinstance(values, Mapping) or not isinstance(next_nodes, (list, tuple)) or not isinstance(interrupts, (list, tuple)):
        ...
        if snapshot.thread_id != thread_id or snapshot.checkpoint_id != row[0]:
...
class CompiledStateGraph:
    ...
    def invoke(
        ...
        *,
        ...
        for event in self.stream(initial, thread_id=thread_id, max_steps=max_steps, resume=resume):
    ...
    def stream(
        ...
        *,
        ...
        for _ in range(max_steps):
            if not next_nodes:
                snapshot = GraphSnapshot(thread_id, step, state, ())
