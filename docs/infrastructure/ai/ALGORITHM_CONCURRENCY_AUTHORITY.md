...
- durable recovery isolates one exact step transaction while preserving every pre/post-step durable write;
...
## Reviewed lower-bound migrations

Five owner symbols intentionally require O(N) work because fail-closed construction or binding must inspect variable-length authority input:
...
- `PersistedQualifiedModelEndpointBinding.binding_for`: every canary bound to the requested deployment/role;
