...
The canonical common-path module is `research_platform.portfolio.api`. `ProjectIdentity` identifies one versioned project. `ProjectManifest` groups project metadata, explicit capability binding inputs, project-owned method requirements, content-addressed configuration references, and study references.
