...
The receipt schema is `research-platform.npe-clean-room.v2` and records the generated template profile. `npe_verified` remains false unless the complete Section-37/40 acceptance sequence passes. The default installed-artifact path must be `template_profile=author`; it may not fall back to provider-first scaffolding. While producer-owned Level-0 compilation/standard bindings are incomplete, the verifier preserves `DOCTOR_BLOCKED:level0_standard_bindings` and `URE_LEVEL0_STANDARD_BINDINGS_UNAVAILABLE` rather than manufacturing an Operator-side compiler or reference lifecycle.
