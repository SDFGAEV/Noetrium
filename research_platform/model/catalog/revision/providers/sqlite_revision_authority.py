...
def _pairs(pairs: list[tuple[str, JsonNode]]) -> dict[str, JsonNode]:
    ...
    for key, value in pairs:
        if key in result:
            raise ModelRevisionIntegrityError("revision payload contains duplicate JSON keys")
        result[key] = value
    return result
...
def _loads(raw: bytes, *, field: str) -> dict[str, JsonNode]:
    ...
    except (UnicodeDecodeError, json.JSONDecodeError, ModelRevisionIntegrityError) as exc:
        if isinstance(exc, ModelRevisionIntegrityError):
            raise
        raise ModelRevisionIntegrityError(f"{field} is not canonical JSON") from exc
...
def _bytes(value: dict[str, JsonNode]) -> bytes:
    try:
        ...
        raise ModelRevisionIntegrityError("revision payload cannot be canonically encoded") from exc
