...
def _audit_study_definition_boundary(root: Path) -> list[SourceInvariantViolation]:
    study = root / "research_platform" / "experimentation" / "study"
...
def _audit_checkpoint_operation_backbone(root: Path) -> list[SourceInvariantViolation]:
    checkpoint = root / "research_platform" / "experimentation" / "checkpoint"
    rows: list[SourceInvariantViolation] = []
...
def _audit_participant_operation_backbone(root: Path) -> list[SourceInvariantViolation]:
    operations = root / "research_platform" / "execution" / "participants"
    contract = root / "research_platform" / "participant" / "core" / "api" / "runtime_operations.py"
    rows: list[SourceInvariantViolation] = []
